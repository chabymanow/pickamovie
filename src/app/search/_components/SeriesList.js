...
export default function MovieList({ series_list }){
    ...
    return(
            <section>
              ...
              {series_list.map((item) => (
                <Link key={`${item.mediaType ?? "unknown"}-${item.id}`} href={`/series/viewDetails/${item.id}`} className="block w-full">

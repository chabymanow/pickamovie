...
export default function SeriesReviews({ series_reviews }){
    const [isOpen, setIsOpen] = useState(false);
    return(
 <div>
      <h2 className="text-3xl font-bold mt-8 mb-4 text-slate-800">Reviews <span className="text-xl font-light">({series_reviews.length})</span></h2>
      ...
      <div className="relative">
        ...
        >
          {series_reviews.map((review) => (
            <div key={review.id} className="w-full p-2 md:p-3 lg:p-5 border border-slate-300 rounded-xl flex flex-col">
              ...
              <div className="prose prose-slate max-w-none dark:prose-invert text-sm md:text-md lg:text-lg reviewText">
        ...
        {!isOpen && (
          <div className="pointer-events-none absolute bottom-0 left-0 w-full h-30 bg-linear-to-t from-white to-transparent backdrop-blur-[1px]" />
        )}
      </div>
